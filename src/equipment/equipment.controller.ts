...
export class EquipmentController {
  ...
  async getEquipmentCategoryInfo(
    @Res() res: Response,
    @Param('category_id') category_id: string
  ) {
    try {
      const result = await this.equipmentService.getEquipmentCategoryInfo(
        category_id
      );
      return res.status(HttpStatus.OK).json({
        status: HttpStatus.OK,
  ...
  async getEquipmentProjectInfo(
    @Res() res: Response,
    @Param('project_id') project_id: string
  ) {
    try {
      const result = await this.equipmentService.getEquipmentProjectInfo(
        project_id
      );
      return res.status(HttpStatus.OK).json({
        status: HttpStatus.OK,
